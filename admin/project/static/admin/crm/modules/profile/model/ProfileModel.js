Ext.define("Crm.modules.profile.model.ProfileModel", {
  ...
  beforeSave: function(data, callback) {
    ...
    if (me.user && me.user.id) {
      me.src.db
        .collection("admin_users")
        .findOne({ login: data.login }, { _id: 1 }, function(e, dt) {
  ...
  $getMyInfo: function(data, callback) {
    ...
    if (me.user && me.user.id) {
      me.src.db
        .collection("admin_users")
        .findOne({ _id: me.user.id }, {}, function(e, dt) {
          if (dt) me.prepRecord(dt, callback);
          else callback(null);
        });
    }
  },
