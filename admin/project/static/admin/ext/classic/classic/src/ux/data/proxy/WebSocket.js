Ext.define ('Ext.ux.data.proxy.WebSocket', {
    extend: 'Ext.data.proxy.Server', //'Ext.data.proxy.Proxy' ,
	...
	runTask: function (action, operation, callback, scope) {
        ...
        if(data._params && data._params.query) {
            if(!data._filters) data._filters = [];
            data._filters.push({_property: 'query', _value: data._params.query})
        }
