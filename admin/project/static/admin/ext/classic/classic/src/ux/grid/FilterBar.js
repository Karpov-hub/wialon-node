...
Ext.define('Ext.ux.grid.FilterBar', {
	...
	updateBuffer				: 2000,					// buffer time to apply filtering when typing/selecting
	...
	showHideButtonIconCls		: 'x-fa fa-search',//filter',				// button iconCls
	...
	clearAllButtonIconCls		: 'x-fa fa-times',//clear-filters', 		// css class with the icon of the clear all button
	...
	parseInitialFilters: function() {
		...
		me.filterArray = [];
		me.grid.store.getFilters().each(function(filter) {
	...
	renderFilterBar: function(grid) {
		...
		me.columns.eachKey(function(key, column) {
            ...
            field.on('change', function(el) {me.applyDelayedFilters(el)})
	...
	applyFilters: function(field) {
		...
		if(!Ext.isEmpty(newVal)) {
            ...
            for(var i = 0;i<me.filterArray.length;i++){
                if(me.filterArray[i]._property == column.dataIndex) {
	...
	setVisible: function(visible) {
		...
		me.grid.headerCt.resumeLayouts();
		me.visible = visible;
	}
});