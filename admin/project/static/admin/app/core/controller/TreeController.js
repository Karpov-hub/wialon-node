Ext.define('Core.controller.TreeController', {
    ...
    ,removePage: function() {
        ...
        var me = this
            ...
            ,id = form.down("[name="+me.model.idField+"]").getValue()
    ...
    ,dropPage: function (node, data, overModel, pos, e) {         
        ...
        for(var i=0;i<data.records.length;i++) {
            rec[me.model.idField] = data.records[i].data.id
            rec.alias = data.records[i].data.alias
            if(rec[me.model.idField] != rec.pid) {
                recs.push(rec)
            }
        }
            ...
            me.model.reorder({
                ...
                if(out && out.data) for(var i = 0;i<out.data.length;i++) {
                    for(var j = 0;j<data.records.length;j++) {
                        if(data.records[j].data.id == out.data[i][me.model.idField]) {
