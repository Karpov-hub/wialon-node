...
Ext.define('Core.controller.Controller', {
    ...
    ,removeFormRecord: function(win) {
        var me = this
            ,form = win.down('form')
            ,data = form.getValues();
        me.removeRows([data[this.model.idField]], function() {
    ...
    ,getRecord: function(rec, callback) {
        ...
        var me = this
        if(me.LocalFormData || !rec || !rec.data || !rec.data[me.model.idField]) {
            callback(rec)           
        } else {
            me.model.readRecord(rec.data[me.model.idField], function(data) {
                for(var i in data) 
                    rec.data[i] = data[i]
    ...
    ,modify: function(rec, formClassName, innerCall, callback) {    
        ...
        var me = this
            ,_id = (rec.data[me.model.idField]? rec.data[me.model.idField] : (rec.data.id? rec.data.id : null))
            ,desktop = this.app.getDesktop()
    ...
    ,remove: function(win) {
        ...
        if(sm.selected.length>0) { 
            for(var i=0;i<sm.selected.length;i++) {
                data.push(sm.selected.items[i].data[me.model.idField])
        ...
        var saveF = function(data) {
            me.model.write(data, function(data, err) {
                ...
                if(closewin) win.close()
                    ...
                    if(!!form.setRowId) {
                        if(data.record && data.record[me.model.idField] && !form.rowId) form.setRowId(data.record[me.model.idField])
                    } else {
                        var idf = form.down('[name='+me.model.idField+']')
                        if(data.record && data.record[me.model.idField] && idf.getValue()=='') idf.setValue(data.record[me.model.idField])   
                    }
    ...
    ,copyRecord: function(menu) {  
        ...
        Ext.clipboard[Object.getPrototypeOf(this.model).$className] = menu.record.data[this.model.idField];
    ...
    ,cutRecord: function(menu, action) {  
        if(!Ext.clipboard) Ext.clipboard = {}
        var oo = {indx: menu.record.data.indx || 0, action: action};
        oo[this.model.idField] = menu.record.data[this.model.idField];
        Ext.clipboard[Object.getPrototypeOf(this.model).$className] = oo;
    }
    ...
    ,pasteRecord: function(menu) {
        ...
        if(Ext.clipboard && Ext.clipboard[modelName]) {
            if(Ext.clipboard[modelName].action == 'copy')
                me.model.copy(Ext.clipboard[modelName][me.model.idField], function(data) {
                    me.refresh()
            ...
            if(Ext.clipboard[modelName].action == 'cut') {
                var oo = {indx: Ext.clipboard[modelName].indx};
                oo[me.model.idField] = Ext.clipboard[modelName][me.model.idField];
