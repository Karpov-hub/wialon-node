Ext.define("Core.controller.ControllerImages", {
    ...
    me.control(win, {
      "[action=upload]": {
        change: function(th, val) {
          me.upload(th.up("grid"), th);
        }
      },
      "[action=removePhoto]": {
        click: function(th, val) {
          me.delPhoto(th.up("grid"));
        }
      },
      "[action=removeAllPhoto]": {
        click: function(th, val) {
          me.delAllPhoto(th.up("grid"));
        }
      }
    });
  ...
  upload: function(grid, inp) {
    ...
    if (inp.fileInputEl.dom.files.length > 0) {
      ...
      Glob.Ajax.uploadFiles(
        inp.fileInputEl.dom.files,
        "/admin.models.fileOperations:upload/" + sizes + "/",
        function(data) {
          if (data && data.length) {
            var s = grid.getStore(),
              l = sizes.split("x");
            for (var i = 0; i < data.length; i++) {
              s.add({
                preview: "/tmp/" + data[i].name + (l > 2 ? "_small" : ""),
                img: data[i].name
              });
            }
          }
