...
Ext.define("Core.data.Synchronization", {
  ...
  getCollectionMax: function(col, field, cb) {
    this.src.db
      .collection(col)
      .findOne({}, { sort: [[field, "desc"]] }, function(e, d) {
        if (d && d[field]) cb(d[field]);
        else cb(null);
      });
  },
  ...
  getAccessedCollections: function(userId, cb) {
    ...
    [
      function(next) {
        me.callModel("Admin.model.User.getUserInfo", { auth: userId }, function(
          usr
        ) {
          ...
          function(next) {
            if (!d.stime) {
              ...
              me.src.db
                ...
                .updateOne(
                  oo,
                  { $set: { stime: d.stime, ltime: d.ltime } },
                  function() {
                    next();
                  }
                );
  ...
  $getLocalRecords: function(data, cb, conn) {
    ...
    [
      function(next) {
        me.callModel(
          "Desktop.modules.users.model.GroupsModel.$getModules",
          { auth: me.user.id },
          function(models) {
            next(models);
          }
  ...
  syncRecordOnServer: function(data, cb, conn) {
    var me = this;
    me.callSelfMethod(data, "saveRecordOnServer", conn, "binnary", function(
      res
    ) {
      cb(res);
    });
    ...
    [
      ...
      function(permis, next) {
        ...
        if (data.data.data.removed) {
          if (permis.del) me.removeRecord(data.data, next);
          else cb({ success: false });
        } else me.upsertRecord(data.data, permis, next);
      },
  ...
  removeRecord: function(data, cb) {
    var set = {};
    for (var i in data.data) if (i != this.idField) set[i] = data.data[i];
    var oo = {};
    oo[this.idField] = data.data[this.idField];
    this.src.db.collection(data.collection).updateOne(oo, set, function(e, d) {
      cb(!e);
    });
  },
  ...
  upsertRecord: function(data, permis, cb) {
    if (permis.add && permis.modify) {
      ...
      this.src.db
        ...
        .updateOne(oo, data.data, { upsert: 1 }, function(e, d) {
          cb(!e);
        });
    } else if (permis.add && !permis.modify) {
