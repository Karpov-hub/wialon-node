...
Ext.define('Core.Controller',{
     ...
     * @param {String} path
     */
    ,setCookie: function(key, value, exdays, domain, path, HttpOnly) {
    ...
    ,tplApply: function(tplName, data, callback) {
        ...
        if(!!callback) 
            ...
            return {
                sendHTML: function() {
                    me.tplReadAndApply(path + '/' + method[1] + '.tpl', data, function(html) {
                        me.sendHTML(html)    
                    })
                },
                sendXML: function() {
                    me.tplReadAndApply(path + '/' + method[1] + '.tpl', data, function(xml) {
                        me.sendXML(xml)    
                    })
                }
            }
