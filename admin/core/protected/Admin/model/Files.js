    ...
    [
      ...
      function(_id, call) {
        ...
        me.src.db
          ...
          .findOne({ _id: _id }, fields, function(e, data) {
      ...
      function(data, mtime, call) {
        ...
        if (params.size == "main") {
          if (data["preview"] && data["preview"].buffer)
            call(data["preview"], mtime);
          else if (data["img"] && data["img"].buffer) call(data["img"], mtime);
          else if (data) call({ buffer: data }, mtime);
          else noImg(null);
        } else if (data[params.size] && data[params.size].buffer)
          call(data[params.size], mtime);
        else noImg(null);
      },
      function(data, mtime) {
      ...
      function(call) {
        if (params.tmpName) {
          ...
          if (params.dir) {
            fs.rename(
              dir + token,
              me.config.staticDir + "/" + params.dir + "/" + token,
              function(err) {
                if (err) callback(null);
                else call(me.config.staticDir + "/" + params.dir + "/" + token);
              }
            );
          } else call(dir + "/" + params.tmpName);
        } else {
          if (params.dir) dir = me.config.staticDir + "/" + params.dir;
