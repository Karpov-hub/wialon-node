...
class Report extends Base {
    //13. Отчёт по преждевременному заезду/подъезду выезду АТТ АО "Каражанбасмунай" в гараж
  ...
  async generateReportTemplate(resourceId) {
    var generateReportParams = 
    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n":"13. Отчёт по преждевременному заезду/подъезду выезду АТТ АО «Каражанбасмунай» в гараж, создан с помощью Repogen.",
        "ct":"avl_unit_group",
        ...
        "tbl":[
          ...
          },
          {
            "n":"unit_group_trips",
            "l":"Поездки",
            "c":"[\"user_column\",\"user_column\",\"time_begin\",\"time_end\",\"location_begin\"]",
            "cl":"[\"Группа/Автоколонна\",\"Дата\",\"Начало движения \",\"Конец движения\",\"Название геозоны (гаража)\"]",
            "cp":"[{\"p\":\"profile_field(model)\",\"vt\":\"0\"},{\"p\":\"(time_begin)\",\"vt\":\"30\"},{},{},{}]",
            "s":"",
            "sl":"",
            "filter_order":[
              "custom_sensors_col",
            ...
            "p":"{\"grouping\":\"{\\\"type\\\":\\\"unit\\\"}\",\"duration\":{\"min\":10,\"flags\":1},\"custom_sensors_col\":[\"\"],\"geozones_ex\":{\"zones\":\"229\",\"types\":\"1\",\"split\":1,\"flags\":0}}",
            "sch":{
              "f1":540,
              "f2":0,
              "t1":1080,
  ...
  make(xlsxData) {
    ...
    xlsxData.forEach((row) => {
      this.insertOneTable("Отчёт по преждевременному заезду/подъезду выезду",row.data, row.headers, row.total, styles);
    })
  }
}
