...
class Report extends Base {
    //12. Отчет об отработанном времени ТС ТОО «Каражанбасмунай»
  ...
  async getData(params) {
    ...
    stringForHeader = `12. Отчет об отработанном времени ТС ТОО «Каражанбасмунай» по объекту ${nameUnit} с ${GlobalStartDate.getDate()}-${GlobalStartDate.getMonth()+1 > 9 ? GlobalStartDate.getMonth()+1 : "0" + (GlobalStartDate.getMonth()+1).toString()}-${GlobalStartDate.getFullYear()} по ${GlobalEndDate.getDate()}-${GlobalEndDate.getMonth()+1 > 9 ? GlobalEndDate.getMonth()+1 : "0" + (GlobalEndDate.getMonth()+1).toString()}-${GlobalEndDate.getFullYear()}`;
  ...
  async generateReportTemplate(resourceId) {
    var generateReportParams = 
    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n":"12. Отчет об отработанном времени ТС ТОО «Каражанбасмунай», создан с помощью Repogen.",
        "ct":"avl_unit",
        ...
        "tbl":[
          ...
          },
          {
            "n":"unit_engine_hours",
            "l":"Моточасы",
            "c":"[\"user_column\",\"time_begin\",\"time_end\",\"duration_move\",\"duration_prev\",\"location_end\",\"mileage\",\"fuel_consumption_fls\"]",
            "cl":"[\"Группа/ Автоколонна\",\"Время зажигания двигателя\",\"Время отключения двигателязажигания двигателя\",\"Время в движении\",\"Время на стоянке\",\"Место положение стоянки\",\"Пройдённое расстояние (Пробег км)\",\"Потрачено ГСМ ( в литрах)\"]",
  ...
  insertOneTable(ws, tableData, headers, total, styles) {
    ...
    headers.unshift("№");
    if (headers.includes("Потрачено ГСМ ( в литрах)")){
      headers[headers.indexOf("Потрачено ГСМ ( в литрах)")] = "Расход ГСМ (в литрах)"
    }
    headers.forEach((chs) => {
      if (chs == "Grouping") chs = "Дата";
  ...
  make(xlsxData) {
    ...
    let ws = this.addWorksheet("Отчет об отработанном времени ТС");
    let column = 1, style = styles.cellContent;

    if (xlsxData && xlsxData.length && xlsxData[0].data && xlsxData[0].data.length){
        ws.cell(1, 1, xlsxData[0].data.length + 4, xlsxData[0].headers.length + 1)
        .string("")
        .style(style);
        ws.cell(row, column, row, xlsxData[0].headers.length + 1, true)
        .string(stringForHeader)
        .style(style);
        row++;
        ...
        .string("Нет данных за выбранный период")
        .style(style);
    }
