...
let GlobalEndDate;
let stringForHeader;
let checkedCProperty = false;
const regex = new RegExp('(\ km)|(\ l)', 'gm');
const regexForRound = new RegExp('/[\D]+/g');
const indexToRound = [8];

class Report extends Base {
    //23. Ежедневное отработанное время за первую смену ТС ТОО  «Каражанбасмунай»
  ...
  async getAllDataFromReport(resultExecReport) {
    ...
    for (const element of resultExecReport.reportResult.tables) {
      ...
      this.deleteWrongTurn(dataOfTable);
      objectToPush.total = ["-----", "Итого", dataOfTable[0].c[2], dataOfTable[0].c[3].t, dataOfTable[0].c[4].t, dataOfTable[0].c[5], dataOfTable[0].c[6], dataOfTable[0].c[7]];
  ...
  async generateReportTemplate(resourceId) {
    var generateReportParams = 
    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n":"23. Ежедневное отработанное время за первую смену ТС ТОО  «Каражанбасмунай», создан с помощью Repogen.",
        "ct":"avl_unit_group",
        ...
        "tbl":[
          ...
          },
          {
            ...
            "cl":"",
            "cp":"",
            "s":"[\"address_format\",\"time_format\",\"us_units\",\"skip_empty_rows\",\"shifts\"]",
            "sl":"[\"Address\",\"Time Format\",\"Measure\",\"Пропускать пустые строки\",\"Смены\"]",
            "filter_order":[
              
            ],
            "p":"{\"address_format\":\"1255211008_10_5\",\"time_format\":\"%E %b %Y_%H:%M:%S\",\"us_units\":0,\"shifts\":\"[{\\\"nm\\\":\\\"Смена1\\\",\\\"tm\\\":28800,\\\"dur\\\":35999},{\\\"nm\\\":\\\"Смена2\\\",\\\"tm\\\":64800,\\\"dur\\\":28799},{\\\"nm\\\":\\\"Смена3\\\",\\\"tm\\\":7200,\\\"dur\\\":21599}]\"}",
            "sch":{
              "f1":0,
              "f2":0,
              "t1":0,
          ...
          },
          {
            ...
            "l":"Ежедневное отработанное время за первую смену ТС ТОО ArgymakTransService",
            "c":"[\"user_column\",\"time_begin\",\"time_end\",\"mileage\",\"duration_ival\",\"fuel_consumption_fls\"]",
            "cl":"[\"Группа/ Автоколонна\",\"Начало движения\",\"Конец движения\",\"Пройденное расстояние\",\"Отработанно время\",\"Потрачено по ДУТ\"]",
            ...
            "p":"{\"grouping\":\"{\\\"type\\\":\\\"total\\\",\\\"sortColumn\\\":-1,\\\"hide\\\":1,\\\"nested\\\":{\\\"type\\\":\\\"shift\\\",\\\"nested\\\":{\\\"type\\\":\\\"unit\\\"}}}\",\"custom_sensors_col\":[\"\"]}",
            "sch":{
              "f1":0,
              "f2":0,
              "t1":0,
  ...
  make(xlsxData) {
    ...
    xlsxData.forEach((row) => {
      this.insertOneTable("Ежедневное отработанное время за первую смену",row.data, row.headers, row.total, styles);
    })
  }
}
