...
const indexToRound = [4, 5, 6, 8, 9, 12];

class Report extends Base {
    //25. Расход топлива АТТ АО  «Каражанбасмунай»
  ...
  async mergeAllToFirstList(allData){
    ...
    allData[0].headers.splice(6,0,"Заправки(дата, время)");
    allData[0].headers.splice(10,0,"Сливы(дата, время)");
    for (let i = 0; i < allData[0].data.length; i++){ 
        allData[0].data[i].c.splice(5,0,allData[1].data[i].c[2]);
        allData[0].data[i].c.splice(9,0,allData[2].data[i].c[2]);
    }
    return;
  }
  ...
  async generateReportTemplate(resourceId) {
    var generateReportParams = 
    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n":"25. Расход топлива АТТ АО  «Каражанбасмунай», создан с помощью Repogen.",
        "ct":"avl_unit_group",
        ...
        "tbl":[
          ...
          },
          {
            ...
            "l":"Сводка",
            "c":"[\"user_column\",\"fuel_level_begin\",\"fuel_level_end\",\"filled\",\"fuel_consumption_fls\",\"fuel_consumption_rates\",\"user_column\",\"thefted\",\"mileage\",\"eh\",\"in_motion\"]",
            "cl":"[\"Группа/Автоколонна\",\"Нач. уровень\",\"Остаток топлива\",\"Заправлено топлива\",\"Факт. Расход\",\"Нормы расхода\",\"Отклонение от норм, %\",\"Слито ГСМ\",\"Пробег\",\"Время работы двигателя\",\"Время в движении\"]",
            "cp":"[{\"p\":\"profile_field(model)\",\"vt\":\"0\"},{},{},{},{},{},{\"p\":\"(((fuel_consumption_fls) - (fuel_consumption_rates)) * const100) / (fuel_consumption_rates)\",\"m\":\"%\",\"vt\":\"0\"},{},{},{},{}]",
            "s":"",
            "sl":"",
            "filter_order":[
              "base_eh_sensor",
  ...
  make(xlsxData) {
    ...
    xlsxData.forEach((row) => {
      this.insertOneTable("Расход топлива",row.data, row.headers, row.total, styles);
    })
  }
}
