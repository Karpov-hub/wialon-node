...
const indexToRound = [5];

class Report extends Base {
    //5. Детализация по заправкам ГСМ АТТ АО Каражанбасмунай
  ...
  async generateReportTemplate(resourceId) {
...

    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n":"5. Детализация по заправкам ГСМ АТТ АО  «Каражанбасмунай», создан с помощью Repogen",
        "ct":"avl_unit_group",
        ...
        "tbl":[
          ...
          },
          {
            ...
            "l":"Заправки",
            "c":"[\"user_column\",\"absolute_mileage_begin\",\"filled\"]",
            "cl":"[\"Группа/ Автоколонна\",\"Пробег\",\"Заправлено\"]",
  ...
  insertOneTable(nameOfSheets, tableData, headers, total, styles) {
    ...
    .string("5. Детализация по заправкам ГСМ АТТ АО Каражанбасмунай в разрезе по дням (в литрах)")
    .style(style);
    row++;
  ...
  make(xlsxData) {
    ...
    xlsxData.forEach((row) => {
      this.insertOneTable("Заправки топлива",row.data, row.headers, row.total, styles);
    })
  }
}
