...
class Report extends Base {
    //8. Ежедневный отчет АТТ  Каражанбасмунай по посещению запретных геозон
  ...
  async generateReportTemplate(resourceId) {
    var generateReportParams = 
    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n":"8. Ежедневный отчет АТТ АО  «Каражанбасмунай» по посещению запретный геозон, создан с помощью Repogen",
        "ct":"avl_unit_group",
        ...
        "tbl":[
          ...
          },
          {
            "n":"unit_group_trips",
            "l":"Поездки",
            "c":"[\"user_column\",\"location_end\",\"time_begin\",\"time_end\",\"duration\"]",
            "cl":"[\"Группа/Атоколона\",\"Наименование запретной геозоны\",\"Начало движения\",\"Конец движения\",\"Проведенное время\"]",
            ...
            "p":"{\"duration\":{\"min\":10,\"flags\":1},\"custom_sensors_col\":[\"\"],\"geozones_ex\":{\"zones\":\"892,891\",\"types\":\"1,1\",\"split\":1,\"flags\":0}}",
            "sch":{
              "f1":0,
              "f2":0,
              "t1":0,
  ...
  make(xlsxData) {
    ...
    xlsxData.forEach((row) => {
      this.insertOneTable("Запретные зоны",row.data, row.headers, row.total, styles);
    })
  }
}
