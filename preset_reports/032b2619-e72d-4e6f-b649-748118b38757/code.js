...
const h_t = {
  ...
  report_name_value: {
    en: "Change in the volume of fuel in the tank depending on the temperature",
    ru: "Изменение объема топлива в баке в зависимости от температуры"
  }
};
let row = 2;
...
class Report extends Base {
  async getData({ lang: inputLang = "en", startDateMill, endDateMill, unit }) {
    lang = inputLang;
    ...
    try {
      ...
      await this._cleanUpResult(); // done
      ...
      // let allUnits = await this.getUnit(params.unit);
      // Select units of one group's from all units
      // await this.parseAllUnitsData([allUnits]);
  ...
  // async getAllUnitOfGroups(groupId) {
  //   const params = {
  //     spec: {
  ...
  // async getUnit(idOfUnit) {
  //   const params = {
  //     id: idOfUnit,
  //     flags: 4097
  //   };
  //   const response = await this.callService("core/search_item", params);
  //   return response.item;
  // }

  // async parseAllUnitsData(arrayWithUnits) {
  //   for (let unit of arrayWithUnits) {
  //     let flagOfCalculate = false;
  //     let keysOfItem = Object.keys(unit.sens);
    ...
    const params = {
      ...
      tbl: [
        ...
        },
        {
          ...
          cp: "",
          s:
            '["report_name","unit_name","current_time","time_begin","time_end"]',
          sl: `[\"${h_t.report_name[lang]}\",\"${h_t.unit_name[lang]}\",\"${h_t.current_time[lang]}\",\"${h_t.time_begin[lang]}\",\"${h_t.time_end[lang]}\"]`,
          filter_order: [],
          p: "",
          sch: {
            f1: 0,
  ...
  async _changeOneObject(inputObject, dateFrom) {
    ...
    for (let element of inputObject.c) {
      ...
      if (value.length == 0) {
        ...
        value = Number(newVal) * Number(temperatureCoef);
      }
  ...
  _deleteUselessParameters(arr) {
    arr.forEach((item) => {
      ...
      delete item.d;
      delete item.uid;
      if (item.r && item.r.length) this._deleteUselessParameters(item.r);
    });
  }
