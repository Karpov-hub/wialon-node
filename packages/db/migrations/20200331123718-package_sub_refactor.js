...
module.exports = {
  up: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.transaction(t => {
      return Promise.all([
        queryInterface.sequelize.query('DROP VIEW usage_reports', {transaction: t}),
        queryInterface.changeColumn( "package_subscriptions", "from_date", { type: Sequelize.DATEONLY }, { transaction: t }),
        queryInterface.changeColumn( "package_subscriptions", "to_date", { type: Sequelize.DATEONLY }, { transaction: t }),
  ...
  down: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.transaction((t) => {
      return Promise.all([
        // queryInterface.sequelize.query(
        //   'CREATE VIEW usage_reports',{ transaction: t }), 
        queryInterface.changeColumn(          
          "package_subscriptions", "from_date", {
            type: Sequelize.DATE,
          },
            { transaction: t }
        ),
        queryInterface.changeColumn(
          "package_subscriptions", "to_date", {
            type: Sequelize.DATE,
          },
            { transaction: t }
        ),
        queryInterface.sequelize.query(
          'drop view if exists usage_reports',{ transaction: t }),     
      ]);  
    });
  }
};
