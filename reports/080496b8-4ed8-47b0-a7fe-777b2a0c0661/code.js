...
const regexForRound = new RegExp('^\d+(?:[\.,]\d+)?$');
const indexToRound = [4];
let GlobalEndDate;

class Report extends Base {
    //2. Детализация расхода топлива АТТ АО Каражанбасмунай в разрезе по дням
  ...
  async generateReportTemplate(resourceId) {
    var generateReportParams = 
    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n": "2. Детализация расхода топлива АТТ АО  «Каражанбасмунай»,  создан с помощью Repogen",
        "ct": "avl_unit_group",
        ...
        "tbl": [
          ...
          },
          {
            ...
            "l": "Моточасы",
            "c": "[\"user_column\",\"fuel_consumption_fls\"]",
            "cl": "[\"Группа/Автоколонна\",\"Общий расход GPS ДУТ\"]",
  ...
  insertOneTable(nameOfSheets, tableData, headers, total, styles) {
    ...
    .string("2. Детализация расхода топлива АТТ АО Каражанбасмунай в разрезе по дням")
    .style(style);
    row++;
  ...
  insertOneObject(inputObject, styles, ws){
    ...
    for (let element of inputObject.c) {
      ...
      if (indexToRound.includes(column)){
  ...
  make(xlsxData) {
    ...
    xlsxData.forEach((row) => {
      this.insertOneTable("Расхода топлива",row.data, row.headers, row.total, styles);
    })
  }
}
