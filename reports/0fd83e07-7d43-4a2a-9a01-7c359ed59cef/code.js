...
class Report extends Base {
    //3. Детализация максимальной скорости АТТ АО «Каражанбасмунай»
  ...
  async generateReportTemplate(resourceId) {
    var generateReportParams = 
    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n":"3. Детализация максимальной скорости АТТ АО «Каражанбасмунай», создан с помощью Repogen",
        "ct":"avl_unit_group",
        ...
        "tbl":[
          ...
          },
          {
            "n":"unit_group_trips",
            "l":"Поездки",
            "c":"[\"user_column\",\"max_speed\"]",
            "cl":"[\"Группа/Автоколонна\",\"Макс. скорость\"]",
            ...
            "p":"{\"grouping\":\"{\\\"type\\\":\\\"unit\\\",\\\"nested\\\":{\\\"type\\\":\\\"day\\\"}}\",\"duration\":{\"min\":10,\"flags\":1},\"speed\":{\"max\":150},\"custom_sensors_col\":[\"\"]}",
            "sch":{
              "f1":0,
              "f2":0,
              "t1":0,
  ...
  insertOneTable(nameOfSheets, tableData, headers, total, styles) {
    ...
    .string("Детализация максимальной скорости АТТ АО Каражанбасмунай")
    .style(style);
    row++;
  ...
  make(xlsxData) {
    ...
    xlsxData.forEach((row) => {
      this.insertOneTable("Детализация максимальной скорости",row.data, row.headers, row.total, styles);
    })
  }
}
