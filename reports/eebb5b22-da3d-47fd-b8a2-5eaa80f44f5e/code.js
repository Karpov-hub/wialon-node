...
class Report extends Base {
    //1. Детализация по пробегу АТТ АО Каражанбасмунай
  ...
  async generateReportTemplate(resourceId) {
    var generateReportParams = 
    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n": "1. Детализация по пробегу АТТ АО «Каражанбасмунай», создан с помощью Repogen", 
        "ct": "avl_unit_group", 
        "p": 
        "{\"descr\":\"\",\"bind\":{\"avl_unit_group\":[]}}", 
        "tbl": [
            { 
                "n": "unit_group_stats", 
                "l": "Статистика", 
                ...
                "n": "unit_group_trips", 
                "l": "Поездки", 
                "c": "[\"user_column\",\"mileage\"]", 
                "cl": "[\"Группа/Автоколонна\",\"Пробег\"]", 
                "cp": "[{\"p\":\"profile_field(model)\",\"vt\":\"0\"},{}]", 
                "s": "", 
                "sl": "", 
                "filter_order": ["custom_sensors_col", "duration", "mileage", "base_eh_sensor", "engine_hours", "speed", "stops", "sensors", "sensor_name", "driver", "trailer", "geozones_ex"], 
  ...
  insertOneTable(nameOfSheets, tableData, headers, total, styles) {
    ...
    .string("1. Детализация по пробегу АТТ АО Каражанбасмунай в разрезе по дням")
    .style(style);
    row++;
    style = styles.headers
    headers.forEach((chs) => {
      if (chs == "Grouping") chs = "Наименование АТТ";
      if (chs == "Пробег") chs = "Общий Пробег по GPS"
  ...
  insertOneObjectHorizontal(inputObject, styles, ws, row){
    ...
    let value = inputObject.c[3].toString();
  ...
  make(xlsxData) {
    ...
    xlsxData.forEach((row) => {
      this.insertOneTable("Общий пробег",row.data, row.headers, row.total, styles);
    })
  }
}
