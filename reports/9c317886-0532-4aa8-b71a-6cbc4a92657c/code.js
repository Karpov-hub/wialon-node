...
class Report extends Base {
  ...
  async parseDateForUnit(arrayWithData, itemId, arrayWithName){
    ...
    for(let item of arrayWithData.messages){
      ...
      if (item && item.p && item.p.ble_str_1 && isUnitCharging == false){
        let arrayOfMinimum = await this.findIndexAboutZeroValue(item.p), indexOfMinimum = null;
        if (arrayOfMinimum && arrayOfMinimum.length == 1 ){
          indexOfMinimum = arrayOfMinimum[0];
        } else {
          indexOfMinimum = await this.selectMinimum(tempRoom, tempCode, arrayOfMinimum)
        }
  ...
  async selectMinimum(tempRoom, tempCode, indexOfMinimum){
    let countForIndex = 1, indexMin = null;
  ...
  async selectObjectToPush(tempMinimumValue) {
    let minimumObject = null;
    for (let item of tempMinimumValue){
      if (item.value < 0) item.value * -1;
      for (let itemForCompare of tempMinimumValue){
        if (itemForCompare.value < 0) itemForCompare.value * -1;
        if ((item.value < itemForCompare.value && minimum == null) || (minimum && item.value < minimum)) {
          minimumObject = item; 
        }
      }
    }
  ...
  async parseDateForUnit(arrayWithData, itemId, arrayWithName){
    ...
    let count = 0;
    let tempTime = 0, tempRoom = null, tempCode = null;
    let isUnitCharging = false, skipTenValue = false, selectedObject = null;
    for (let i = 0; i < arrayWithName.messages.length; i++){
      if (i % 10 == 0 && !skipTenValue){
        selectedObject = await this.selectObjectToPush(tempMinimumValue);
        for (let j = 0; j < 10; j++){
          if (objectToReturn.sumTime == undefined){
        ...
        skipTenValue = true; 
        tempMinimumValue = [];
      } else if(i != arrayWithName.messages.length - 1){
        skipTenValue = false;
        let arrayOfMinimum = await this.findIndexAboutZeroValue(item.p), indexOfMinimum = null;
        if (arrayOfMinimum && arrayOfMinimum.length == 1 ){
          tempMinimumValue.push({index: arrayOfMinimum[0], value: arrayWithData.messages[i].p['ble_str_'+arrayOfMinimum[0]], timestamp: arrayWithData.messages[i].p.t});
        } else {
          indexOfMinimum = await this.selectMinimum(tempRoom, tempCode, arrayOfMinimum)
          tempMinimumValue.push({index: indexOfMinimum, vaule: arrayWithData.messages[i].p['ble_str_'+indexOfMinimum], timestamp: arrayWithData.messages[i].p.t});
        }
      } else {
    ...
    for (let itemUnit of group.u){
      ...
      if (messages && messages.count == 0){
        ...
        if (parsedMessages.length == 0){
          ...
          // let mergeAllWhatNeeds = await this.mergeRepeatRoomRecords(deletedTenOrTwentyArray);
          // let mergaAllWhatNeedsLatest = await this.mergeRecordWithSameRoomId(mergeAllWhatNeeds);
          firstObject.startTime = parsedMessages[0].startTime;
          firstObject.endTime = parsedMessages[parsedMessages.length-1].endTime;
          objectToPush.messages = parsedMessages;
        }
      }
    ...
    for (let i = 0; i < messages.length; i++){
      if (i != 0){
        if (messages[i].sumTime == 10 || messages[i].sumTime == 20){
          ...
          messages[i-1].endTime = this.getTimeString(messages[i-1].endTimeNum, false);
    ...
    for (let i = 0; i < messages.length; i++){
      if (messages[i].sumTime > 90) {
        ...
        if (messages[i].sumTime <= 90 && arrayRoomNumbers.indexOf(messages[i].room_id) > -1) {
          ...
          messages[indexToSum].sumTimeString = this.getTimeString(messages[indexToSum].endTimeNum - messages[indexToSum].startTimeNum, true) || "---"
          messages.splice(i, 1);
          i -= 2;
    ...
    for (let i = 0; i < messages.length; i++){
      if (i!=0){
        if (messages[i].sumTime && messages[i].room_id && messages[i].room_id == messages[i-1].room_id &&
            messages[i].startTimeNum - messages[i-1].endTimeNum <= 60){
          messages[i-1].sumTime += messages[i].sumTime;
