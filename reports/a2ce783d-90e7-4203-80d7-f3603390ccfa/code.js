...
const indexToRound = [7];
class Report extends Base {
    //14. Поездки по объектам АТТ АО  «Каражанбасмунай»
  ...
  async getData(params) {
    ...
    stringForHeader = `14. Поездки по объекту АТТ АО  «Каражанбасмунай» с ${GlobalStartDate.getDate()}-${GlobalStartDate.getMonth()+1 > 9 ? GlobalStartDate.getMonth()+1 : "0" + (GlobalStartDate.getMonth()+1).toString()}-${GlobalStartDate.getFullYear()} по ${GlobalEndDate.getDate()}-${GlobalEndDate.getMonth()+1 > 9 ? GlobalEndDate.getMonth()+1 : "0" + (GlobalEndDate.getMonth()+1).toString()}-${GlobalEndDate.getFullYear()}`;
  ...
  async generateReportTemplate(resourceId) {
    var generateReportParams = 
    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n":"14. Поездки по объектам АТТ АО  «Каражанбасмунай», создан с помощью Repogen.",
        "ct":"avl_unit",
        ...
        "tbl":[
          ...
          },
          {
            ...
            "cl":"",
            "cp":"",
            "s":"[\"trips_count\",\"duration\",\"mileage\",\"max_speed\"]",
            "sl":"[\"Кол-во поездок\",\"Длительность поездок\",\"Пробег за период\",\"Макс. скорость в поездках\"]",
            "filter_order":[
          ...
          },
          {
            "n":"unit_trips",
            "l":"Поездки",
            "c":"[\"user_column\",\"time_begin\",\"time_end\",\"duration\",\"mileage\",\"fuel_consumption_fls\",\"max_speed\"]",
            "cl":"[\"Группа/Автоколонна\",\"Старт поездки\",\"Окончание поездки\",\"Длительность поездки\",\"Пройденное расстояние\",\"Расход в ГСМ\",\"Максимальная скорость\"]",
  ...
  make(xlsxData) {
    ...
    let ws = this.addWorksheet("Поездки по объектам");
    let column = 1, style = styles.cellContent;
    if (xlsxData && xlsxData.length && xlsxData[0].data && xlsxData[0].data.length && xlsxData[0].headers && xlsxData[0].headers.length){
