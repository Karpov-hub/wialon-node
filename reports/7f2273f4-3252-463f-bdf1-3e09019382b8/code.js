...
const indexToRound = [6];

class Report extends Base {
    //18. Суточный пробег ТС  «Каражанбасмунай»
  ...
  async getData(params) {
    ...
    try {
      ...
      await this.mergeSecondListWithFirst(allData);
  ...
  async mergeSecondListWithFirst(allData){
    allData[0].headers.push("Последнее сообщение");
    for (let i = 0; i < allData[0].data.length; i++){
        allData[0].data[i].c.push(allData[1].data[i].c[1]);
    }
    return;
  }
  ...
  async generateReportTemplate(resourceId) {
    var generateReportParams = 
    {
        ...
        "itemId": resourceId,
        "callMode": "create",
        "n":"18. Суточный пробег ТС  «Каражанбасмунай», создан с помощью Repogen.",
        "ct":"avl_unit_group",
        ...
        "tbl":[
          ...
          },
          {
            "n":"unit_group_trips",
            "l":"Поездки",
            "c":"[\"user_column\",\"mileage\",\"avg_fuel_consumption_rates\",\"fuel_consumption_all\"]",
            "cl":"[\"Группа/ Автоколонна\",\"Пройденное расстояние\",\"Норма расхода ГСМ\",\"Расхлод ГСМ\"]",
  ...
  make(xlsxData) {
    ...
    xlsxData.forEach((row) => {
      this.insertOneTable("Суточный пробег",row.data, row.headers, row.total, styles);
    })
  }
}
